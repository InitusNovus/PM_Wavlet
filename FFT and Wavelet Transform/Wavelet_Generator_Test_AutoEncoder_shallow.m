...
%% Import Data
dataImportTimer = tic;
dataPath = "../Data/Wavelet_Result_Transient/Wavelet_ReIm";
dataRaw = struct;
...
%%
L_ch = 1;
aeDataMat = squeeze(responses(:, :, L_ch, :));
aeData = cell(1, size(aeDataMat, 3));

for k = 1:size(aeDataMat, 3)
    aeData{k} = aeDataMat(:,:,k);
end
%% AutoEncoder Training
% Training options
hiddenSize = 100;
autoenc = trainAutoencoder(aeData, hiddenSize,...
        'MaxEpochs', 100000,...
...
XTest = aeData{k};
figure(1);
imagesc(XTest); % Plot the CWT coefficients using imagesc
colormap(jet); % Choose the colormap for the CWT coefficients plot
xlabel('Time');
ylabel('Frequency');
axis('tight')
title("Wavelet Data 250 x 87 (ABS), Predict")
colorLimits = [min(XTest(:)) max(XTest(:))]; % Get color limits for the original data

XReconstructed = predict(autoenc, XTest);
figure(2);
imagesc(XReconstructed); % Plot the CWT coefficients using imagesc
colormap(jet); % Choose the colormap for the CWT coefficients plot
xlabel('Time');
ylabel('Frequency');
axis('tight')
title("Wavelet Data 250 x 87 (ABS), Reconstructed")

% Compare color limits from reconstructed data
colorLimits = [min([colorLimits(1), min(XReconstructed(:))]) max([colorLimits(2), max(XReconstructed(:))])];

% Apply the same color limits to both figures
caxis(gca, colorLimits);
figure(1);
caxis(gca, colorLimits);
colorbar; % Display the color bar after setting color limits to make sure it's consistent
%%
...
% set(gca, 'YScale', 'log'); % Set the y-axis to log scale to better visualize the frequency components
axis('tight')
title("Wavelet Data 250 x 87 (Real), GT")
...
% set(gca, 'YScale', 'log'); % Set the y-axis to log scale to better visualize the frequency components
axis('tight')
title("Wavelet Data 250 x 87 (Imag), Predict")
...
% set(gca, 'YScale', 'log'); % Set the y-axis to log scale to better visualize the frequency components
axis('tight')
title("Wavelet Data 250 x 87 (Imag), GT")

%
reconCfs.Real = double(YPred(:,:,1));
reconCfs.Imag = double(YPred(:,:,2));
...
reconCfs.complex = complex(reconCfs.Real, reconCfs.Imag);
...
function layers = createTConvFinal(numOutputChannel, dropoutFcl, dropoutTConv, dropoutResize,namePrefix)
layers = [
    transposedConv2dLayer([7 7], 128, "Name", namePrefix + "_in")
    batchNormalizationLayer();
    leakyReluLayer()
    dropoutLayer(dropoutTConv)
    ...
    resize3dLayer("OutputSize", [160 160 32], 'Method','trilinear')
    batchNormalizationLayer();
    leakyReluLayer()
    dropoutLayer(dropoutResize)
    ...
    resize3dLayer("OutputSize", [75 238 32], 'Method','trilinear')
    batchNormalizationLayer();
    leakyReluLayer()
    ...
    resize3dLayer("OutputSize", [87 250 numOutputChannel], 'Method','trilinear', 'Name', namePrefix + "_out")];
end
